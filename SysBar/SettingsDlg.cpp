...
#define IDC_PARAM_BASE      300   // 300..(300+P_COUNT-1)
...
// Internal helpers
// ─────────────────────────────────────────────────────────────────────────────
static void PopulateDeviceCombo(HWND hCombo,
    const std::vector<DeviceInfo>& devices,
    const std::string& currentPath,
    const wchar_t* offlineMsg)
{
...
// Window procedure
// ─────────────────────────────────────────────────────────────────────────────
static LRESULT CALLBACK SettingsWndProc(HWND hDlg, UINT msg,
    WPARAM wParam, LPARAM lParam)
{
    SettingsDlgCtx* ctx = reinterpret_cast<SettingsDlgCtx*>(
    ...
    case WM_CREATE:
    {
        ...
        HWND lblPort = CreateWindowExW(0, L"STATIC", L"LHM Port:",
            WS_CHILD | WS_VISIBLE,
            10, y + 3, 70, 18, hDlg, nullptr, cs->hInstance, nullptr);
        SendMessageW(lblPort, WM_SETFONT, (WPARAM)hFont, TRUE);

        HWND editPort = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            WS_CHILD | WS_VISIBLE | ES_NUMBER,
            85, y, 60, 22,
            hDlg, (HMENU)IDC_PORT_EDIT, cs->hInstance, nullptr);
        ...
        HWND lblNote = CreateWindowExW(0, L"STATIC",
            L"(Default 8085. Match LHM \u2192 Options \u2192 Remote Web Server)",
            WS_CHILD | WS_VISIBLE,
            152, y + 4, 350, 16,
            hDlg, nullptr, cs->hInstance, nullptr);
        SendMessageW(lblNote, WM_SETFONT, (WPARAM)hFont, TRUE);
        y += 34;

        HWND lblSep = CreateWindowExW(0, L"STATIC",
            L"\u2500\u2500\u2500 Display Parameters (max 10) "
            L"\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500",
            WS_CHILD | WS_VISIBLE,
            10, y, 490, 16,
            hDlg, nullptr, cs->hInstance, nullptr);
        ...
        for (int i = 0; i < P_COUNT; ++i)
        {
            ...
            HWND chk = CreateWindowExW(0, L"BUTTON",
                ...
                10 + col * 250, y + row * 22, 240, 20,
                hDlg,
                (HMENU)(UINT_PTR)(IDC_PARAM_BASE + i),
                cs->hInstance, nullptr);
            SendMessageW(chk, WM_SETFONT, (WPARAM)hFont, TRUE);
            SendMessageW(chk, BM_SETCHECK,
                ctx->s->cfg.enabled[i] ? BST_CHECKED : BST_UNCHECKED, 0);
        }
        y += ((P_COUNT + 1) / 2) * 22 + 10;

        HWND lblGpu = CreateWindowExW(0, L"STATIC", L"GPU:",
            WS_CHILD | WS_VISIBLE,
            10, y + 3, 45, 18,
            hDlg, nullptr, cs->hInstance, nullptr);
        SendMessageW(lblGpu, WM_SETFONT, (WPARAM)hFont, TRUE);

        HWND gpuCombo = CreateWindowExW(0, L"COMBOBOX", L"",
            WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
            60, y, 440, 200,
            hDlg, (HMENU)IDC_GPU_COMBO, cs->hInstance, nullptr);
        SendMessageW(gpuCombo, WM_SETFONT, (WPARAM)hFont, TRUE);
        PopulateDeviceCombo(gpuCombo, ctx->gpus, ctx->s->cfg.gpuPath,
            L"(LHM offline \u2014 start LHM and reopen Settings)");
        y += 30;

        HWND lblDisk = CreateWindowExW(0, L"STATIC", L"Disk:",
            WS_CHILD | WS_VISIBLE,
            10, y + 3, 45, 18,
            hDlg, nullptr, cs->hInstance, nullptr);
        SendMessageW(lblDisk, WM_SETFONT, (WPARAM)hFont, TRUE);

        HWND diskCombo = CreateWindowExW(0, L"COMBOBOX", L"",
            WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
            60, y, 440, 200,
            hDlg, (HMENU)IDC_DISK_COMBO, cs->hInstance, nullptr);
        SendMessageW(diskCombo, WM_SETFONT, (WPARAM)hFont, TRUE);
        PopulateDeviceCombo(diskCombo, ctx->disks, ctx->s->cfg.diskPath,
            L"(LHM offline \u2014 start LHM and reopen Settings)");
        y += 30;

        HWND lblNic = CreateWindowExW(0, L"STATIC",
            L"Network Adapters  (Ctrl+Click to select multiple):",
            WS_CHILD | WS_VISIBLE,
            10, y, 400, 16,
            hDlg, nullptr, cs->hInstance, nullptr);
        ...
        HWND nicList = CreateWindowExW(WS_EX_CLIENTEDGE, L"LISTBOX", L"",
            WS_CHILD | WS_VISIBLE | WS_VSCROLL |
            LBS_MULTIPLESEL | LBS_NOINTEGRALHEIGHT,
            10, y, 490, 72,
            hDlg, (HMENU)IDC_NIC_LIST, cs->hInstance, nullptr);
        SendMessageW(nicList, WM_SETFONT, (WPARAM)hFont, TRUE);

        if (ctx->nics.empty())
        {
            SendMessageW(nicList, LB_ADDSTRING, 0,
                (LPARAM)L"(LHM offline \u2014 start LHM and reopen Settings)");
        }
        else
        {
            ...
            {
                SendMessageW(nicList, LB_ADDSTRING, 0,
                    (LPARAM)ctx->nics[i].name.c_str());
                if (selPaths.count(ctx->nics[i].path))
                    SendMessageW(nicList, LB_SETSEL, TRUE, i);
            }
        }
        y += 76;

        HWND btnAll = CreateWindowExW(0, L"BUTTON", L"Select All",
            WS_CHILD | WS_VISIBLE,
            10, y, 90, 22,
            hDlg, (HMENU)IDC_SELECT_ALL_NIC, cs->hInstance, nullptr);
        SendMessageW(btnAll, WM_SETFONT, (WPARAM)hFont, TRUE);

        HWND btnClear = CreateWindowExW(0, L"BUTTON", L"Clear All",
            WS_CHILD | WS_VISIBLE,
            108, y, 90, 22,
            hDlg, (HMENU)IDC_CLEAR_NIC, cs->hInstance, nullptr);
        SendMessageW(btnClear, WM_SETFONT, (WPARAM)hFont, TRUE);
        y += 30;

        HWND lblStatus = CreateWindowExW(0, L"STATIC", L"",
            WS_CHILD | WS_VISIBLE,
            10, y, 490, 18,
            hDlg, (HMENU)IDC_STATUS_LABEL, cs->hInstance, nullptr);
    ...
    case WM_COMMAND:
    {
        ...
        if (id == IDC_SAVE_BTN)
        {
            ...
            {
                SetDlgItemTextW(hDlg, IDC_STATUS_LABEL,
                    L"Invalid port. Enter a value between 1 and 65535.");
                return 0;
            }
            ...
            if (colCount > MAX_COLS)
            {
                wchar_t msg[128];
                swprintf_s(msg,
                    L"Selection would require %d columns (max %d). "
                    L"Please deselect some parameters.",
                    colCount, MAX_COLS);
            ...
            for (int i = 0; i < P_COUNT; ++i)
            {
                HWND chk = GetDlgItem(hDlg, IDC_PARAM_BASE + i);
                ctx->s->cfg.enabled[i] =
                    (SendMessageW(chk, BM_GETCHECK, 0, 0) == BST_CHECKED);
            }

            if (!ctx->gpus.empty())
            {
                int sel = (int)SendDlgItemMessageW(hDlg, IDC_GPU_COMBO,
                    CB_GETCURSEL, 0, 0);
                ctx->s->cfg.gpuPath =
                    (sel > 0 && sel - 1 < (int)ctx->gpus.size())
                    ? ctx->gpus[sel - 1].path : "";
            }

            if (!ctx->disks.empty())
            {
                int sel = (int)SendDlgItemMessageW(hDlg, IDC_DISK_COMBO,
                    CB_GETCURSEL, 0, 0);
                ctx->s->cfg.diskPath =
                    (sel > 0 && sel - 1 < (int)ctx->disks.size())
                    ? ctx->disks[sel - 1].path : "";
            }
            ...
            if (IsWindow(ctx->s->hwnd))
            {
                ...
                ResizeSwapChain(ctx->s,
                    (UINT)ctx->s->widgetW, (UINT)ctx->s->widgetH);
    ...
    case WM_CLOSE: DestroyWindow(hDlg); return 0;
        ...
        // Centralized memory cleanup ties context directly to window destruction lifecycle
    case WM_DESTROY:
    {
        if (ctx)
        {
...
{
    if (s->hwndSettings && IsWindow(s->hwndSettings))
    {
        SetForegroundWindow(s->hwndSettings);
        return;
    }
    ...
    HWND hDlg = CreateWindowExW(
        ...
        WC_SETTINGS, L"SysBar \u2014 Settings",
        WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_VISIBLE,
        CW_USEDEFAULT, CW_USEDEFAULT, 0, 0,
        nullptr, nullptr, s->hInst, ctx);
